-- Tremega Bids & Estimates Platform - Complete Database Schema
...
CREATE OR REPLACE FUNCTION calculate_bid_totals()
RETURNS TRIGGER AS $$
DECLARE
...
-- Trigger to recalculate bid totals when line items change
CREATE TRIGGER recalculate_bid_totals
  AFTER INSERT OR UPDATE OR DELETE ON line_items
  FOR EACH ROW EXECUTE FUNCTION calculate_bid_totals();
...
-- View for bid summaries with line item counts
CREATE OR REPLACE VIEW bid_summaries AS
SELECT 
  b.id,
  b.title,
  b.status,
  ...
  COUNT(CASE WHEN li.assigned_sub_id IS NOT NULL THEN 1 END) as assigned_items_count
FROM bids b
LEFT JOIN line_items li ON b.id = li.bid_id
GROUP BY b.id, b.title, b.status, b.total, b.customer_name, b.created_at;

-- View for sub performance summary
...
SELECT 
  ...
  COUNT(CASE WHEN sa.status = 'started' THEN 1 END) as active_assignments
FROM subs s
LEFT JOIN sub_assignments sa ON s.id = sa.sub_id
GROUP BY s.id, s.name, s.email, s.rating, s.performance_score, s.total_jobs_completed;
...
COMMENT ON TABLE bids IS 'Main bids/estimates table';
COMMENT ON TABLE line_items IS 'Line items for each bid';
COMMENT ON TABLE subs IS 'Subcontractors database';
COMMENT ON TABLE sub_assignments IS 'Assignments of subs to line items';
COMMENT ON TABLE notifications IS 'User notifications';
COMMENT ON TABLE messages IS 'Messages between users';
COMMENT ON TABLE photos IS 'Photos uploaded for bids/jobs';
